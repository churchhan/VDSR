...
if __name__ == '__main__':
	...
	if not USE_QUEUE_LOADING:
		...
		train_input  	= tf.placeholder(tf.float32, shape=(BATCH_SIZE, IMG_SIZE[0], IMG_SIZE[1], 1), name="input")
		...
		train_input_single  = tf.placeholder(tf.float32, shape=(IMG_SIZE[0], IMG_SIZE[1], 1),name="input")
	...
	saver = tf.train.Saver(weights, max_to_keep=0, write_version = saver_pb2.SaverDef.V1)
	...
	with tf.Session(config=config) as sess:
		...
		if USE_QUEUE_LOADING:
			...
			for epoch in range(0, MAX_EPOCH):
				...
				tf.train.write_graph(sess.graph, '/home/jin/sr/graph', 'test_epoch_%03d.pbtxt'% epoch)

				#converter = tf.lite.TFLiteConverter.from_session(sess, [input], [out])
				#tflite_model = converter.convert()
				#open("/home/jin/sr/sr.tflite", "wb").write(tflite_model)
